...
# *** display ***
# highest growth schools with high poverty rates -- median income -- minority rates -- gender gaps -- enrollment
merged <- merged %>% 
  mutate(enroll_group = ifelse(enrollment < 6000, 0, 1), avg_class_size = enrollment/teacher, staff_ratio = num_staff/enrollment) %>% 
  filter(avg_class_size < 40)
...
highest_growth <- cohort_3_5 %>% 
  arrange(desc(combined_35_growth))

 
gt(highest_growth) %>% 
  tab_header(
    title= "Most Efficient CA School Districts") %>% 
  fmt_number(columns = c(3:5),
             decimals = 1)
