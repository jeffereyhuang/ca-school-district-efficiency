...
test <- clean_growth %>% 
  filter(cohort == 2007, subject == "math") 
# 3-8
# 3-8 math
# 3-8 english
# 3-5
...
cohort_3_8 <- clean_growth %>% 
  ...
  mutate(avg_growth_38yr = mean(c(ela, math), na.rm = TRUE))
...
cohort_3_5 <- clean_growth %>% 
  ...
  mutate(avg_growth_35yr = mean(c(ela, math), na.rm = TRUE))
...
cohort_6_8 <- clean_growth %>% 
  ...
  group_by(leaidC, leaname) %>% 
  mutate(ela = mean(ela), math = mean(math)) %>% 
  mutate(avg_growth_68yr = mean(c(ela, math), na.rm = TRUE))
